...
class Analyser:
    """
    Find owners of companies in OCOD file
    by Panama Papers graph DB
    """
    @classmethod
    def get_all_companies(cls, file) -> dict:
        """
        1 Step
    ...
    @classmethod
    def check_for_company(cls, file, companies: dict) -> list:
        """
        2 Step
    ...
    @classmethod
    def _find_owner(cls, node: str, db: dict):
        """
        find owner by id
        :param node: str:
        :param db: dict:
        ...
        if edge:
            if 'shareholder' in edge[1] or 'owner' in edge[1]:
                ...
                return cls._find_owner(edge[0], db)
            else:
                return None

    @classmethod
    def find_owner_ids(cls, file, companies: list) -> list:
        """
        ...
        for company in companies:
            company['Owner'] = cls._find_owner(company['Node_Id'], dict_db)
        return companies

    @classmethod
    def find_names_by_id(cls, file, ix_companies: dict):
        """
    ...
    def main(cls):
        ...
        with open('src/Panama_papers/panama_papers.nodes.entity.csv', 'r') as f:
            ix_companies = cls.find_names_by_id(f, ix_companies)
